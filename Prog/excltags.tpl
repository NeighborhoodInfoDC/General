...
define tagset Tagsets.ExcelBase;
    ...
    /*--------------------------------------------------------1Aug 05-*/
    log_note = "NOTE: This is the Excel XP tagset (Compatible with SAS 9.1.3 and above, v1.131, 04/23/2015). Add options(doc='help') to the ods statement for more information.";
    ...
    define event changes;
        start:
            set $changelog[] '-';
            set $changelog[] '.v1.131, 04/23/2015';
    ...
    */
    ...
    define event needed_styles;
        ...
        set $missing_styles['linecontent']  'True';
    ...
    define event write_tabs_entry;
        ...
        do /if index($worksheetName, " ") or index($worksheetName,"=");
    ...
    define event contents_entry;
        ...
        break / if cmp(event_name,"page_setup");
   ...
   /*************************************************************************/
   /*  Modified the output generated with the contents= option to also add  */
   /*  quotes when the bygroups are created similar to when the  INDEX=     */
   /*  is used and contains the "="                                         */
   /*************************************************************************/
    ...
    define event empty_style;
            ...
            do /if $doit;
                ...
                trigger create_just_cell_styles;
                    ...
                    cmp($cell_class, 'linecontent')|   /* Added     */
   ...
   /*------------------------------------------*/
   /*-- Added the just "d" as a valid category */
   /*------------------------------------------*/
    ...
    /*-------------------------------------------------*/
    /*-- Restricted the styles which include the __d --*/
    /*-- in the style name to only the data: styles. --*/
    /*-------------------------------------------------*/

    define event just_cell_style;
        break / if !contains($cell_class,"data") and cmp($just,"d");
    ...
    define event style_class;
        ...
        /*************************************************************/
        /*--26Sep14                                                --*/
        /*-- Added the class linecontent when the minimize_style   --*/
        ...
        do /if $minimize_style;
            ...
            set $doit "true" /if cmp($htmlclass, "linecontent");
        ...
        */
        ...
        /*-------------------------------------------------------chevell--*/
    /*-- Added linecontent to the list of class names that append   --*/
    /*   the justification to the name.                             --*/
        /*----------------------------------------------------26Sep14-----*/
...
/*        putlog "GET_BORDERWIDTH:";
    ...
    define event font_interior;
       ...
       /*-------------------------------------------------chevell--*/
         /*-- Excel doesn't honor font families. This step extracts--*/
         /*   the first font on this list. The list also causes    --*/
         /*   an error when the length is greater than 31          --*/
       /*   S1099348                                             --*/
       /*------------------------------------------------31Jul14 --*/

         do /if  index($fontFace, ",") > 0;
             set $fontFace scan($fontFace, 1,",");
         else;
            ...
            /*---------------------------------------------------------114Oct14--
            /*-- Getting the font height of the the linecontent to used late to --*/
            /*-- compute the row height of the line statementof PROC REPORT.    --*/
        /*-----------------------------------------------------------------------*/

        do / if cmp(htmlclass,"linecontent") or cmp(htmlclass,"notecontent");
             eval $line_point_size $font_height;
        done;
    ...
    /*-------------------------------------------------------chevell------*/
    /* Modified to correct duplicate worksheet names added when         --*/
    /* the sheet name is #byval and frozen_headers is set to an integer.--*/
    /*-------------------------------------------------------------1Apr15-*/

        do /if $name_count;
       do / if contains(lowcase($sheet_name),"#byval") and
               cmp($sheet_interval,"bygroup") and
               exist($frozen_header_count) or
               exist($frozen_rowheader_count);

           eval $name_count $name_count -2;
           do / if $name_count > 0;
...
/*            trigger check_set_byvars;*/
    ...
    define event verbatim;
        ...
        finish:
            ...
            /*-----------------------------------------------------chevell----*/
        /*-- Footnotes are already being triggered and causes duplicate --*/
        /*-- footnotes in the output. Commenting the trigger to the     --*/
        /*     embedded_footnotes event.                                --*/
        /*--------------------------------------------------5Jfeb 15------*/
    ...
    define event table;
        ...
        define event span_cell_row;
            ...
            finish:
                ...
                /*****************************************************************/
                /* Creating new variable $span_cell_row which has the row of the */
                /* last data note added with the compute before page.            */
                /*****************************************************************/

                set $span_cell_row $worksheet_row;
        end;
        ...
        define event data_note;
            start:
                ...
                /*--******************************************************************--*/
        /*-- Modifying how the row height is calculated when a line statement --*/
        /*-- is added fixing issues assoctaed with S0855202                   --*/
                /*--***********************************************************Chevell--*/

                do / if cmp(event_name,"data_note");
                   do / if ^$line_point_size;
            eval $line_height $point_height;

                   else;
               eval $line_height $tmp_row_height * $line_point_size+6;
           done;
        done;
      ...
      finish:
          ...
          /*----------------------------------------------------19Aug03-*/
            ...
            /*-------------------------------------------Chevell--*/
            /* Added Unsetting the $line_height variable created--*/
            /*--------------------------------------------16Oct14-*/
            unset $line_height;
     ...
     define event row_start;
            ...
            do /if ^$do_auto_fit_height;
                ...
                /*------------------------------------------------Chevell-*/
                /*- Recalculating the row height for the line statement --*/
                /*------------------------------------------------14Oct14-*/

                do / if $line_height;
             set $this_row_height $line_height;

        else;
             set $this_row_height $this_row_height;
                done;
    ...
    */
    ...
    define event calculate_header_len;
        ...
        /*--------------------------------------------Chevell----*/
        /*-- Unset the variable $loc_function and $loc_style   --*/
       ...
       /*-----------------------------------------------------------------Chevell-*/
       /*-- Get rid of the namespace information so that we can get the correct --*/
       /*-- column width.                                                       --*/
       /*---------------------------------------------------------------17Nov14---*/

      do / if contains($tmp_value,"xmlns=""http://www.w3.org/TR/REC-html40""");

         eval $scount count($tmp_value,"xmlns");
         do / if $scount=1 ;

            do / if substr($tmp_value,1,1)="<" ;
                 eval $tmp_value substr($tmp_value,find($tmp_value,">"));
                 eval $tmp_value substr($tmp_value,1,find($tmp_value,"</"));

            else;
                 eval $loc_function find($tmp_value,'<B');
                 eval $loc_style find($tmp_value,'<Font');
                 eval $tmp_value_part1 substr($tmp_value,1,find($tmp_value,"<"));

                 do / if $loc_style > 0;
                      eval $tmp_value_part2  substr($tmp_value,find($tmp_value,">"));
                      eval $tmp_value_part2  substr($tmp_value_part2,1,find($tmp_value_part2,"</"));
                      eval $tmp_value  catt($tmp_value_part1,$tmp_value_part2);
                 else;
                      eval $loc_function reverse($tmp_value);
                      do / if  substr($loc_function,1,1) ne ">";
                           eval $tmp_value_part2 reverse(substr(reverse($tmp_value),1,find($tmp_value,"<")));
                           eval $tmp_value catt($tmp_value_part1,$tmp_value_part2);
                      else;
                           eval $tmp_value $tmp_value_part1;
                      done;
                 done;
            done;
             ...
             eval $loc_function find($tmp_value,"<B");
             do / if substr($tmp_value,1,1)="<";
                 do / if $loc_function = 0;
                     eval $tmp_value substr(substr($tmp_value,find($tmp_value,">")),2);
                     eval $tmp_value substr($tmp_value,find($tmp_value,">"));
                     eval $tmp_value substr($tmp_value,1,find($tmp_value,"<"));

                 else;
                     eval $tmp_value substr(substr($tmp_value,find($tmp_value,">")),2);
                     do / if contains($tmp_value,"<U");
                          eval $tmp_value substr($tmp_value,1,find($tmp_value,"<U"));
                     else;
                         eval $tmp_value substr($tmp_value,1,find($tmp_value,"<B"));
                     done;
                 done;

             else;
                  eval $tmp_value_part1 substr($tmp_value,1,find($tmp_value,"<"));
                  eval $temp find($tmp_value,"<Font");
                  eval $tmp_value_part2 substr($tmp_value,$temp);
                  eval $tmp_value_part2 substr($tmp_value_part2,find($tmp_value_part2,">"));
                  eval $tmp_value_part2 substr($tmp_value_part2,1,find($tmp_value_part2,"</"));
                  eval $tmp_value  catt($tmp_value_part1,$tmp_value_part2);
             done;
                 ...
                 do / if $loc_style < $loc_function;
                      eval $tmp_value substr($tmp_value,1,find($tmp_value,"</B"));
                      eval $tmp_value substr(reverse($tmp_value),find(reverse($tmp_value),"/<"));
                      eval $tmp_value reverse(scan($tmp_value,1,">"));
                 else;
                      eval $tmp_value substr($tmp_value,1,find($tmp_value,"</Font"));
                      eval $tmp_value_part1 substr($tmp_value,1,$loc_function);
                      eval $tmp_value_part2 substr($tmp_value,$loc_style,find($tmp_value,"</Font"));
                      eval $tmp_value_part2  scan(reverse($tmp_value_part2),2,">");
                      eval $tmp_value catt($tmp_value_part1,$tmp_value_part2);
                 done;

           else / if $scount=4;

                   eval $loc_style find($tmp_value,'<U');
                   eval $tmp_value substr($tmp_value,find($tmp_value,"<U"));
               eval $tmp_value substr($tmp_value,find($tmp_value,">"),find($tmp_value,"<B"));
                 eval $tmp_value scan($tmp_value,1,"<");
           done;

           eval $tmp_value strip(translate($tmp_value," ","<>"));
    ...
    define event cell_start;
        start:
           ...
           /*---------------------------------------------------chevell-*/
       /*-- Added linecontent to this list.                       --*/
       /*----------------------------------------------------26Sep--*/
            ...
            do /if any(font_face, font_size, font_style,
                ...
                text_decoration, $cell_wraptext, $vjust,indent
                );
        ...
        finish:
            ...
            do /if $formula;
               ...
               /*********************************************************************-*/
               /*-- Commented out the Do statement which executes for numbers only  --*/
               /********************************************************************--*/
...
<Font html:Face="American Typewriter" html:Color="#000000">age
    ...
    define event style_over_ride;
            ...
            break /if ^$style_over_ride;
                ...
                set $key $key "td" text_decoration "wt" $cell_wraptext "vj" $vjust "ind" indent;
                do /if $debug_level = -9;
                ...
                /*--------------------------------------------------chevell */
                /*-- Added the linecontent class here to the overrides      */
                /*----------------------------------------------------------*/
                ...
                do /if ^$style_overrides[$key] ;
                    ...
                    do /if contains($cell_class, "ata") |
                        ...
                        cmp($cell_class, "notecontent")|  /* Header */
                        cmp($cell_class, "linecontent");  /* Header */
    ...
    define event table_body;
        ...
        do /if $table_count = $autofilter_table;
            do /if ^$autofilter_row;
                ...
                /*****************************************************************/
                /* Set the autofilter row based on the $span_cell_row            */
                /*****************************************************************/

                do / if  $span_cell_row;
                  eval $autofilter_row  $span_cell_row;
                done;
            done;
        done;
        unset $is_a_table_head;
...
/*        unset $footers;*/
    ...
    define event span_cell;
        start:
            ...
            /*---------------------------------------------------chevell--*/
        /*-- Check for $worksheet_widths since batch output will    --*/
        /*-- not have have a value.                                 --*/
            /*------------------------------------------------5Feb 15-----*/
            ...
            eval $worksheet_widths $table_widths;
         done;
     ...
     define event unicode ;
         ...
         start:
            do /if value;
               ...
               eval $unicode inputn($newvalue, "hex.") ;
